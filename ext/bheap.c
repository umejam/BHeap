...
static VALUE bheap_initialize(VALUE self)
{
  BHEAP* pb;
...
static VALUE bheap_push(VALUE self, VALUE v)
{
  ...
  while (c > 0) {
    if (Qtrue == rb_funcall2(pb->buf[p], rb_intern(">="), 1, &v)) {
      break;
    }
    pb->buf[c] = pb->buf[p];
    c = p;
...
static VALUE bheap_pop(VALUE self)
{
  ...
  while (c < pb->length) {
    if (((c + 1) < pb->length) && 
	(Qtrue == rb_funcall2(pb->buf[c], 
			      rb_intern("<"), 
			      1, 
			      (pb->buf + c + 1)))) {
      c = c + 1;
    }
    if (Qtrue == rb_funcall2(v, rb_intern(">="), 1, (pb->buf + c))) {
      break;
    }
    pb->buf[p] = pb->buf[c];
    p = c;
...
void Init_bheap(void)
{
  ...
  rb_define_private_method(BHeap, "initialize", bheap_initialize, 0);
